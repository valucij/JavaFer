...
public class Tree implements ShellCommand{
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		List<String> listArgument = Util.parseArguments(arguments);
		
		if(listArgument.size() != 1) {
			Util.message(env, "1", listArgument.size());
			return ShellStatus.CONTINUE;
		}
	
		Path directory = Util.resolvesPath(env, listArgument.get(0));
		
		try {
			Files.walkFileTree(directory, new TreeVisit(env));
		} catch (IOException e) {
