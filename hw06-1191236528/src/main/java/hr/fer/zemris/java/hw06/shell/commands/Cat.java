...
public class Cat implements ShellCommand{
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		String one = Util.parse(env, arguments, 0);
		
		if(one == null) {
			env.writeln("Invalid use of cat command");
			return ShellStatus.CONTINUE;
		}
		
		Charset charset = null;
		try {
			charset = Util.parseCharset(env, arguments, one.length());//ali ovo je drugačiji parse!!!! za charset napraviti u util-u	
		...
		try {
			...
			while(in.read(reading) != -1) {
				...
				//System.out.printf(" ovo je onaj out u catu ." + out + ".");
				env.writeln(out);
			}
